...
const namesCharacters = require('../../utils/nameChapters.json');

const limitListPerPage = 8;

function getCharacterList(action) {
  return Api.get(
    `characters?limit=${limitListPerPage}&offset=${
      action.payload.page * limitListPerPage
    }`
  );
}
/**
...
function* requestCharacterByNameGeneral(action) {
  ...
  yield all(
    namesCharacters.map((nameList) => {
      return nameList.indexOf(action.payload.name.toLowerCase()) !== -1
        ? call(requestCharacterByName, nameList)
        : true;
    })
  );
...
export function* requestCharacterByName(name) {
  try {
    ...
    const response = yield call(getCharacterByName, nameWithoutSpace);

    if (response && response.data && response.status === 200) {
      yield put({ type: Types.ADD_SEARCH_LIST, payload: response.data });
    } else yield put({ type: Types.FAILURE, error: { message: '' } });
  } catch (error) {
...
function* requestCharactersList(action) {
  try {
    const response = yield call(getCharacterList, action);
